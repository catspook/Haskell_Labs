...
> sortLentOut pat = Patron (name pat) (phone pat) (sort $ lent pat)
...
>   show (Patron name phone lent) = name ++ "\n" ++
>       phone ++ "\n" ++ (concat $ map (\x -> "*" ++ "  " ++ (show x) ++ "\n") lent) 
...
> checkout lib bk pat 
>   | (quantAvailable lib (title bk)) > Just 0 = Just (decrementAvail lib bk, lentToPatron pat bk) 
>   | otherwise = Nothing
