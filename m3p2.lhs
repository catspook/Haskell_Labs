...
deriving Show: tells interpreter to figure out types and how to print it itself. 
(child) node (child)
...
for all a that are able to be ordered relative to one another, 
  we take a binTree and a data and turn it into another binTree
...
Type signature:
    (Ord a) is the type of a
    everything to the left of the last -> are parameters
    the thing to the right of the last -> is what it returns
Node l a r ===>>> Node (BTree a) a (BTree a)
in definition of BTree on line 34
b is what you are inserting, a is root, l is left subtree, r is right subtree
space complexity is linear (2*number of verts)
