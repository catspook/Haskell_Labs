...
fmap jumps the border of the safety box to operate on what's inside it (monads == let in clj)

> whitelist = do
>               w <- fmap Set.fromList (fmap list readFile "whtlist.txt") 
>               t <- fmap Set.fromList (fmap list readFIle "traffic.txt") 
>               return $ getSize (Set.union (Set.fromList (lines w)) t) (Set.fromList (lines w)) 

> getSize x y = Set.size (Set.difference x y)
