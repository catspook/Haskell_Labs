...
The base case Miran LipovaÄa calls an "accumulator" in *Learn You a Haskell*. 
...
> unLine xs = foldl (\acc x -> acc ++ [[x]]) [] xs
...
> sumBool xs = foldl (\acc x -> if (x == True) then (acc + 1) else acc) 0 xs 

> sumBool' xs = (length . filter (== True)) xs 
