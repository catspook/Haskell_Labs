...
looks like a struct, and you can't add things to it, but you can use addr Lib1 to get the address of Library1 like you can with maps in Clojure.
...
> buildLibrary addr tup = Library addr [((Book x y z), 1) | (x, y, z)<- tup] 
...
> getBook lib bk = filter (\x -> (title (fst (x))) == bk) (books lib)

> isNothing lst
>   | lst == [] = Nothing
>   | otherwise = Just $ snd(lst!!0) 

> quantAvailable :: Library -> String -> Maybe Int
> quantAvailable lib bk = isNothing (getBook lib bk) 
...
> lentToPatron person bk = Patron (name person) (phone person) ([bk] ++ (lent person))
...
> decrementAvail lib bk = Library (addr lib) ([if book == bk && num > 0 then (book, (num - 1)) else (book, num) | (book, num) <- books lib])
