...
Prelude> mult3 n = n * 3
**Add > to anything you want to be run as code
...
> profitOrNo income expense = if income > expense then "profit" else "no profit"

Multiline functions: one extra space per indent.

> profitOrNo' income expense =
>  if income > expense
>    then "profit"
>    else "no profit"

no> profitOrNo' income expense
no>  if income > expense
no >  then "profit"
no>  else "no profit"
