...
> squatchCount xs = length (filter (\x -> x == "sasquatch") xs)
...
> remove :: Eq a => a -> [a] -> [a]
> remove word xs = filter (\x -> x /= word) xs 
...
> drySeason xs = (map . remove) "rain" xs
...
> drySeasonCount xss = sum $ map (length . (filter (\x -> x /= "rain")))  xss
